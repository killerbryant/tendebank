...
public class ManagerServiceImpl implements ManagerService {
	...
	public boolean insert(Manager member) {
		...
		try {
			...
			Manager qryManager = getDaoFactory().gethManagerDao().findByKey(memberAccount);
			
			if(ObjectUtils.isObjNotBlank(qryManager)) {
				result = false;
			} else {
				// 系統取編號
				String memberNo = selectMaxManagerNo();
				logger.debug("系統配給會員編號 = " + memberNo);
				// 有取到編號則新增
				if(StringUtils.isNotBlank(memberNo)) {
					member.setManagerNo(memberNo);
					// 密碼MD5加密
					member.setPassword(MD5Utils.md5Encryption(password));
					getDaoFactory().gethManagerDao().insert(member);
				} else {
					result = false;
				}
			}
	...
	public Manager selectManagerByAccount(Manager member) {
		...
		try {
			...
			newManager = getDaoFactory().gethManagerDao().findByKey(memberAccount);
			logger.debug("查詢的帳號" + newManager.getAccount());
		} catch (Exception e) {
			logger.debug("Error");
		}
		return newManager;
	...
	public String selectMaxManagerNo() {
		...
		int maxNum = 0;
		try {
			String maxManagerNo = getDaoFactory().gethManagerDao().findByMaxKey(ConstantUtils.MEMBER_NO_FIRST_WORD+"%");
			logger.debug("最大的會員編號 " + maxManagerNo);
			if(StringUtils.isNotBlank(maxManagerNo)) {
				maxNum = Integer.parseInt(maxManagerNo.substring(1));
			}
	...
	public String queryManagerService(Manager member, String ctxPath) {
		...
		try {
			...
			// 尋找輸入帳號密碼存在
			qryManager = getDaoFactory().gethManagerDao().findByAccountAndMail(member);
			logger.debug("查詢的值" + qryManager.toString());
			if(ObjectUtils.isObjNotBlank(qryManager)) {
				// 比對資料庫PWD和OPWD是否一樣，一樣表密碼查詢，不一樣代表已經申請更換密碼
				if(StringUtils.equals(qryManager.getPassword(), MD5Utils.md5Encryption(qryManager.getOriginalPassword()))) {
					// 產生20碼的新密碼
					String newPwd = CommonUtils.randomString(20);
					
					String msg = "<a href='" + ConstantUtils.DOMAIN_NAME + ctxPath + "/resetPage.action?acc=" + memberAccount + "&email=" + email + "&resetPwd=" + newPwd + "'>密碼重置</a>";
					// 發送更新密碼的mail
					getMailUtils().sendMail(ConstantUtils.SMTP_AUTH_USER, email, "找回密碼", "請點選重置密碼連結更新您的密碼 -> " + msg);
					// member 設定新的密碼
					qryManager.setOriginalPassword(newPwd);
					// 更新密碼
					getDaoFactory().gethManagerDao().update(qryManager);
				} else {
					String oPwd = qryManager.getOriginalPassword();
					String msg = "<a href='" + ConstantUtils.DOMAIN_NAME + ctxPath + "/resetPage.action?acc=" + memberAccount + "&email=" + email + "&resetPwd=" + oPwd + "'>密碼重置</a>";
					// 發送更新密碼的mail
					getMailUtils().sendMail(ConstantUtils.SMTP_AUTH_USER, email, "找回密碼", "請點選重置密碼連結更新您的密碼 -> " + msg);
	...
	public boolean updatePassword(Manager member, String resetPassword) {
		...
		try {
			...
			// 尋找輸入帳號密碼存在
			qryManager = getDaoFactory().gethManagerDao().findByAccountAndMail(member);
			logger.debug(qryManager.getOriginalPassword() + " 亂數產生的密碼 = " + resetPassword);
			if(ObjectUtils.isObjNotBlank(qryManager) && StringUtils.equals(qryManager.getOriginalPassword(), resetPassword)) {
				qryManager.setPassword(MD5Utils.md5Encryption(member.getOriginalPassword()));
				qryManager.setOriginalPassword(member.getOriginalPassword());
				// 更新密碼
				getDaoFactory().gethManagerDao().update(qryManager);
			} else {
				result = false;
